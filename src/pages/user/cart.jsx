import Breadcrumb from "@/components/breadcrumb";
import ServiceFeatures2 from "@/components/service-features2";
import { useState } from "react";
import { ShoppingCart, Trash2, Minus, Plus, Gift, Truck } from "lucide-react";
import ProductImage from "@/assets/product-img.png";
import { useNavigate } from "react-router-dom";

const Cart = () => {
    // Dữ liệu sản phẩm và voucher
    const productsData = [
        {
            number: 1,
            name: "Own The Run",
            image: ProductImage,
            description: "Lightweight and breathable training jersey.",
            price: 399000,
            quantity: 2, // Số lượng đã chọn trong giỏ hàng
            size: "L",
        },
    ];

    const vouchersData = [
        {
            id: 2,
            code: "SUMMER10",
            discountValue: 30000, // 30.000đ
            minOrderValue: 500000,
            startDate: "2025-02-01",
            endDate: "2025-06-30",
        },
    ];
    const codeVoucher = vouchersData[0];
    const [cartItems, setCartItems] = useState(productsData);
    const [voucherCode, setVoucherCode] = useState("");
    const [discount, setDiscount] = useState(0);

    // Cập nhật số lượng sản phẩm
    const updateQuantity = (index, change) => {
        const updatedCart = [...cartItems];
        updatedCart[index].quantity = Math.max(1, updatedCart[index].quantity + change);
        setCartItems(updatedCart);
    };

    // Xóa sản phẩm khỏi giỏ hàng
    const removeItem = (index) => {
        setCartItems(cartItems.filter((_, i) => i !== index));
    };

    // Tính tổng tiền trước giảm giá
    const subtotal = cartItems.reduce((acc, item) => acc + item.price * item.quantity, 0);

    // Kiểm tra điều kiện áp dụng voucher
    const applyVoucher = () => {
        const voucher = vouchersData.find((v) => v.code === voucherCode);
        if (voucher && subtotal >= voucher.minOrderValue) {
            setDiscount(voucher.discountValue);
        } else {
            setDiscount(0);
            alert("Voucher không hợp lệ hoặc đơn hàng chưa đạt mức tối thiểu.");
        }
    };

    // Tính phí ship
    const shipping = subtotal >= 500000 ? 0 : 30000;

    // Tính tổng đơn hàng
    const total = subtotal - discount + shipping;

    const navigate = useNavigate();
    return (
        <div>
            <div className="flex h-[350px] flex-col items-center justify-center gap-[20px] bg-sub-cyan">
                <div className="title-page">Shopping Cart</div>
                <Breadcrumb />
            </div>
            <div className="content mb-[120px] mt-[50px]">
                <h2 className="mb-4 text-xl font-semibold">Shopping Cart</h2>
                <div className="flex justify-between">
                    <div>
                        {/* Bảng giỏ hàng */}
                        <table className="w-[820px] border-collapse overflow-hidden rounded-[8px]">
                            <thead>
                                <tr className="h-[60px] bg-secondary text-[18px] font-normal text-white">
                                    <th className="p-2 font-semibold">Product</th>
                                    <th className="p-2 font-semibold">Price</th>
                                    <th className="p-2 font-semibold">Quantity</th>
                                    <th className="p-2 font-semibold">Subtotal</th>
                                    <th className="p-2 font-semibold"></th>
                                </tr>
                            </thead>
                            <tbody>
                                {cartItems.map((item, index) => (
                                    <tr
                                        key={index}
                                        className="border-b border-gray-300"
                                    >
                                        <td className="flex w-full items-center p-4">
                                            <img
                                                src={item.image}
                                                alt={item.name}
                                                className="h-[110px] w-[90px] rounded-[8px] border border-primary"
                                            />
                                            <div className="ml-3">
                                                <p className="text-[18px] font-semibold">{item.name}</p>
                                                <p className="text-sm text-gray-500">Size: {item.size}</p>
                                            </div>
                                        </td>
                                        <td className="p-2 text-[18px]">
                                            <span className="flex items-center justify-center">{item.price.toLocaleString()} đ</span>
                                        </td>

                                        <td className="w-[140px] p-2">
                                            <div className="flex items-center justify-center">
                                                <button
                                                    onClick={() => updateQuantity(index, -1)}
                                                    className="flex h-[24px] w-[30px] items-center justify-center rounded-[8px] border border-primary bg-white text-primary"
                                                >
                                                    <Minus size={16} />
                                                </button>
                                                <span className="btn mx-2 h-[24px] w-[30px] rounded-[8px] border-primary bg-white text-primary">
                                                    {item.quantity}
                                                </span>
                                                <button
                                                    onClick={() => updateQuantity(index, 1)}
                                                    className="flex h-[24px] w-[30px] items-center justify-center rounded-[8px] border border-primary bg-white text-primary"
                                                >
                                                    <Plus size={16} />
                                                </button>
                                            </div>
                                        </td>

                                        <td className="p-2 text-[18px]">
                                            <span className="flex items-center justify-center">
                                                {(item.price * item.quantity).toLocaleString()} đ
                                            </span>
                                        </td>
                                        <td className="p-2">
                                            <button
                                                onClick={() => removeItem(index)}
                                                className="text-secondary"
                                            >
                                                <Trash2 size={20} />
                                            </button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>

                        <div className="mt-[100px] flex items-center gap-2">
                            {" "}
                            <Gift className="text-secondary" />{" "}
                            <p className="rounded-[5px] border border-primary p-1 font-semibold text-primary"> {codeVoucher.code}</p>{" "}
                            <span className="text-secondary"> For order above 500k </span>{" "}
                        </div>
                        <div className="mt-[40px] flex items-center gap-2">
                            {" "}
                            <Truck className="text-secondary" />{" "}
                            <p className="rounded-[5px] border border-primary p-1 font-semibold text-primary">30.000 đ</p>{" "}
                            <span className="text-secondary"> Freeship for order above 500k </span>{" "}
                        </div>

                        {/* Nhập voucher */}
                        <div className="mt-[40px] flex items-center gap-3">
                            <input
                                type="text"
                                placeholder="Voucher Code"
                                value={voucherCode}
                                onChange={(e) => setVoucherCode(e.target.value)}
                                className="h-[50px] rounded-[8px] border p-2"
                            />
                            <button
                                onClick={applyVoucher}
                                className="h-[47px] rounded-[8px] bg-primary px-4 py-2 text-white"
                            >
                                Apply Voucher
                            </button>
                        </div>
                    </div>
                    {/* Order Summary */}
                    <div className="w-[395px] rounded-[8px] border p-[25px]">
                        <h3 className="mb-[25px] text-lg font-semibold">Order Summary</h3>
                        <div className="sort-line mb-[22px] w-full bg-[#D9D9D9]"></div>
                        <div className="mb-[28px] flex justify-between">
                            <span className="order-title">Items</span>
                            <span className="order-value">{cartItems.length}</span>
                        </div>
                        <div className="mb-[28px] flex justify-between">
                            <span className="order-title">Subtotal</span>
                            <span className="order-value">{subtotal.toLocaleString()} đ</span>
                        </div>
                        <div className="mb-[28px] flex justify-between">
                            <span className="order-title">Shipping</span>
                            <span className="order-value">{shipping === 0 ? "Free" : `${shipping.toLocaleString()} đ`}</span>
                        </div>
                        <div className="mb-[28px] flex justify-between">
                            <span className="order-title">Voucher Discount</span>
                            <span className="text-[18px] text-red-500">-{discount.toLocaleString()} đ</span>
                        </div>{" "}
                        <div className="sort-line mb-[22px] w-full bg-[#D9D9D9]"></div>
                        <div className="mt-2 flex justify-between text-lg font-semibold">
                            <span className="order-title">Total</span>
                            <span className="text-[18px] text-primary">{total.toLocaleString()} đ</span>
                        </div>
                        <button
                            onClick={() => navigate("checkout")}
                            className="btn mt-[26px] h-[50px] w-[346px] rounded-[8px]"
                        >
                            Proceed to Checkout
                        </button>
                    </div>
                </div>
            </div>

            <ServiceFeatures2 />
        </div>
    );
};
export default Cart;
